...
def start(Path,Size,que):
    ...
    def Collect(Host):
        try:
            q.put(Host)
            IP = Host.get('host')
            templates = open("templates/huawei_display_version_patch")
            Connect = base.dev_login(Host,que)
            Sysname = Connect.find_prompt().strip('<>')
            Output = Connect.send_command('display version')
            Output += '\n'+Connect.send_command('display patch')
            Connect.disconnect()
            with open('log/CollectVersion/{}-{}.log'.format(datetime.datetime.now().strftime("%Y-%m-%d-%Hh%Mm%Ss"),IP),'w',newline='',encoding='utf-8') as OutFile:
    ...
    try:
        ...
        with open(datetime.datetime.now().strftime("%Y-%m-%d-%Hh%Mm%Ss")+'版本补丁收集.csv','a',newline='') as AllFile:
            f=csv.writer(AllFile,dialect='excel')
            f.writerow(["设备名称","IP","型号","型号详细","版本","补丁","运行时间"])
            for i in result:
                if i:
                    f.writerow(i)
