...
def start(Path,Size,que):
    ...
    def Collect(Host):
        try:
            q.put(Host)
            IP = Host.get('host')
            templates = open("templates/huawei_display_interface")
            Connect = base.dev_login(Host,que)
            Sysname = Connect.find_prompt().strip('<>')
            Output = Connect.send_command('display interface')
            Connect.disconnect()
            with open('log/CollectIntVer/{}-{}.log'.format(datetime.datetime.now().strftime("%Y-%m-%d-%Hh%Mm%Ss"),IP),'w',newline='',encoding='utf-8') as OutFile:
                OutFile.write(Output)
            fsm = textfsm.TextFSM(templates)#调用TextFSM模板
            fsm_results = fsm.ParseText(Output)#提取设备接口
            if fsm_results:
    ...
    try:
        ...
        with open(datetime.datetime.now().strftime("%Y-%m-%d-%Hh%Mm%Ss")+'接口信息收集.csv','a',newline='') as AllFile:
            f=csv.writer(AllFile,dialect='excel')
            f.writerow(["设备名称","IP","接口","状态","模式","PVID"])
            for i in result:
                if i:
                    f.writerow(i)
