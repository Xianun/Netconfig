...
def start(Path,Size,que):
    ...
    def Collect(Host):
        try:
            q.put(Host)
            IP = Host.get('host')
            templates = open("templates/huawei_display_device_manufacture-info")
            Connect = base.dev_login(Host,que)
            Sysname = Connect.find_prompt().strip('<>')
            Output = Connect.send_command('display device manu')
            Output += '\n'+Connect.send_command('display elabel backplane')
            Output += '\n'+Connect.send_command('display elabel brief')
            Output += '\n'+Connect.send_command('display esn all')
            Connect.disconnect()
            with open('log/CollectSN/{}-{}.log'.format(datetime.datetime.now().strftime("%Y-%m-%d-%Hh%Mm%Ss"),IP),'w',newline='',encoding='utf-8') as OutFile:
                OutFile.write(Output)
            fsm = textfsm.TextFSM(templates)#调用TextFSM模板
            fsm_results = fsm.ParseText(Output)#提取设备SN信息
            if fsm_results:
    ...
    try:
        ...
        with open(datetime.datetime.now().strftime("%Y-%m-%d-%Hh%Mm%Ss")+'设备SN收集.csv','a',newline='') as AllFile:
            f=csv.writer(AllFile,dialect='excel')
            f.writerow(["设备名称","IP","Slot","型号","SN","制造日期"])
            for i in result:
                if i:
                    f.writerow(i)
