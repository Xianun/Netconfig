...
def start(Path,Size,que):
    ...
    def Collect(Host):
        try:
            q.put(Host)
            IP = Host.get('host')
            templates = open("templates/huawei_display_license")
            Connect = base.dev_login(Host,que)
            Sysname = Connect.find_prompt().strip('<>')
            Output = Connect.send_command('display license')
            Connect.disconnect()
            with open('log/CollectLicense/{}-{}.log'.format(datetime.datetime.now().strftime("%Y-%m-%d-%Hh%Mm%Ss"),IP),'w',newline='',encoding='utf-8') as OutFile:
    ...
    try:
        ...
        with open(datetime.datetime.now().strftime("%Y-%m-%d-%Hh%Mm%Ss")+'License收集.csv','a',newline='') as AllFile:
            f=csv.writer(AllFile,dialect='excel')
            f.writerow(["设备名称","IP","文件信息","状态","失效码","产品名称","产品型号","License SN","创建日期","SNS过期日期","特性名","过期日期","试用天数","功能项"])
            for i in result:
                if i:
                    f.writerow(i)
